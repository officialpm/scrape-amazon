...
def extractPage(url: str) -> str:
    ...
    while pageNotLoaded:
        r = get_URL(url)
        productPage = BeautifulSoup(r.text, 'html.parser')
        checkReviewLen = len(productPage.findAll(
            "i", {"class": "review-rating"}))
        if checkReviewLen > 0:
            pageNotLoaded = False
    reviewers = []
    ratings = []
    ...
    reviewTitlesSpan = productPage.findAll(
        "a", {"class": "review-title-content"})
    reviewDescriptionSpan = productPage.findAll(
        "span", {"class": "review-text-content"})

    # Loop is initiated from 2 because we have to exclude the Top Positive and Top Critical Review which otherwise will get repeated.
...
def extractTotalPages(url):
    r = get_URL(url)
    productPage = BeautifulSoup(r.text, 'html.parser')
    pageSpanText = productPage.find(
        "span", {"data-hook": "cr-filter-info-review-count"}).get_text()
